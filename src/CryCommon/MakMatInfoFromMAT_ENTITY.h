...
//	
//	File: MAKEMATINFOFROMMAT_ENTITY.h
//  Description: Material code shared between resource compiler and 3dengine.
//
//	History:
//	- 28/5/2001: Created by Vladimir Kajalin
                                ...
                                CMatInfo & newMat, IRenderer * pRenderer, MAT_ENTITY * me)
{
  ...
  char diffuse[256]="";
  strcpy_s(diffuse, me->map_d.name);

  char bump[256]="";
  strcpy_s(bump, me->map_b.name);

  char normalmap[256]="";
  if(me->map_displ.name[0] && (me->flags & MTLFLAG_CRYSHADER))
    strcpy_s(normalmap, me->map_displ.name);
  ...
  if(me->map_o.name[0])
  {
    if (me->flags & MTLFLAG_CRYSHADER)
      strcpy_s(decal, me->map_o.name);
    else
      strcpy_s(opacity, me->map_o.name);
  }

  char gloss[256]="";
  if(me->map_g.name[0])
    strcpy_s(gloss, me->map_g.name);

  char cubemap[256]="";

  char env[256]="";
  if(me->map_e.name[0])
    strcpy_s(env, me->map_e.name);

  char spec[256]="";
  if(me->map_s.name[0])
    strcpy_s(spec, me->map_s.name);

  char det[256]="";
  if(me->map_detail.name[0])
    strcpy_s(det, me->map_detail.name);

  char subsurf[256]="";
  if(me->map_subsurf.name[0])
    strcpy_s(subsurf, me->map_subsurf.name);

  char refl[256]="";
  if(me->map_e.name[0])
    strcpy_s(refl, me->map_e.name);
