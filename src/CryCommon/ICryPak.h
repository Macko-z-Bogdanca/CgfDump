...
struct ICryPak
{
	...
	// Used for widening fopen_s functionality. They're ignored for the regular File System files.
	enum Efopen_sFlags
	{
		// If possible, will prevent the file from being read from memory.
		fopen_s_HINT_DIRECT_OPERATION = 1,
		// Will prevent a "missing file" warnings to be created.
		fopen_s_HINT_QUIET = 1 << 1
	};
  ...
  virtual FILE *fopen_s(const char *pName, const char *mode, unsigned nFlags = 0)=0;
	virtual FILE *fopen_s(const char *pName, const char *mode,char *szFileGamePath,int nLen)=0;
