...
#include "..\CryCommon\Cry_Geo.h" //#include <Cry_Geo.h>
...
namespace Overlap 
{
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////
	//  Sphere_OBB
	//	VERY IMPORTANT: Sphere is assumed to be in the space of the OBB, 
	...
	inline bool Sphere_OBB( const Sphere &s, const OBB &obb ) 
	{
		...
		//the rest is the same as the "Overlap::Sphere_AABB" calculation
		float quatradius = s.radius * s.radius;
		Vec3 quat(0,0,0);
		AABB aabb=AABB(obb.c-obb.h,obb.c+obb.h);
	...
	*/
	...
	inline bool OBB_Triangle( const Vec3 &pos, const OBB &obb,  const Vec3 &tv0, const Vec3 &tv1, const Vec3 &tv2 ) 
	{
		...
		//    cross-product(edge from triangle, {x,y,z}-direction),  these are 3x3=9 tests
		float min,max,p0,p1,p2,rad,fex,fey,fez;  
		fex = fabsf(e0.x);
		fey = fabsf(e0.y);
		fez = fabsf(e0.z);
	...
	*
	* conventional method to check if two AABB's overlap 
	* or if AABB1 is completely inside AABB2.
	* both AABBs are assumed to be in the same space
	*
	* Example:
