...
MatChunkLoadErrorEnum LoadMatEntity (const CHUNK_HEADER& chunkHeader, const void* pChunkData, unsigned nChunkSize, MAT_ENTITY& me)
{
    ...
    me.m_New = 2;
    strcpy_s(me.name, pMatChunk->name);
    switch (pMatChunk->MtlType)
    {
      case MTL_STANDARD:
        ...
        /* //Timur[10/24/2001] 
        strcpy_s(me.map_a, pMatChunk->tex_a.name);
        strcpy_s(me.map_d, pMatChunk->tex_d.name);
        strcpy_s(me.map_o, pMatChunk->tex_o.name);
        strcpy_s(me.map_b, pMatChunk->tex_b.name);
        strcpy_s(me.map_s, pMatChunk->tex_s.name);
        strcpy_s(me.map_g, pMatChunk->tex_g.name);
        strcpy_s(me.map_c, pMatChunk->tex_c.name);
        strcpy_s(me.map_e, pMatChunk->tex_rl.name);
        strcpy_s(me.map_rr, pMatChunk->tex_rr.name);
        strcpy_s(me.map_det, pMatChunk->tex_det.name);
        */
...
bool CMatEntityNameTokenizer::operator < (const CMatEntityNameTokenizer& right)const
{
	...
	int nComp = _stricmp (this->szTemplate, right.szTemplate);
	...
	nComp = _stricmp (this->szName, right.szName);
