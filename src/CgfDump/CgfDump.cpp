...
#include "CgfUtils.h"
#include "..\CryCommon\StringUtils.h"//#include "..\StringUtils.h"
#include "..\CryCommon\Cry_Geo.h" //#include "..\src\CryCommon\Cry_Geo.h"
#include <set>
...
public:
	...
	TGeomSorter<TVector,float,3>(pVerts, nVerts)
	{
		...
		{
			this->m_arrSortOrder[0] = 0;
			this->m_arrSortOrder[1] = 1;
			this->m_arrSortOrder[2] = 2;
		}
		else
		{
			for (int i = 0; i < 3; ++i)
				switch (szSortOrder[i])
			{
				...
				case 'Y':
				case 'Z':
					this->m_arrSortOrder[i] = szSortOrder[i] - 'X';
				break;
...
void printChunkMtl (const MTL_CHUNK_DESC_0746* pChunk, int nSize)
{
	...
	{
	case MTL_STANDARD:
		printf ("Colors: diffuse "); print (pChunk->col_d);
		printf (", specular "); print (pChunk->col_s);
		printf (", ambient "); print (pChunk->col_a);
		printf ("\nSpecLevel %.2f, SpecShininess %.2f, SelfIllumination %.2f, Opacity %.2f\n", pChunk->specLevel, pChunk->specShininess, pChunk->selfIllum, pChunk->opacity);
		printf ("Ambient", pChunk->tex_a);
		printf ("Diffuse", pChunk->tex_d);
		printf ("Specular", pChunk->tex_s);
		printf ("Opacity", pChunk->tex_o);
		printf ("Bump", pChunk->tex_b);
		printf ("Gloss", pChunk->tex_g);
		printf ("Filter", pChunk->tex_fl);
		printf ("Reflection", pChunk->tex_rl);
		printf ("Subsurface", pChunk->tex_subsurf);
		printf ("Detail", pChunk->tex_det);
		printf ("Flags %08X %s ", pChunk->flags, getMtlFlags(pChunk->flags).c_str());
		printf ("Dyn: Bounce %.3f, StaticFriction %.3f, SlidingFriction %.3f\n", pChunk->Dyn_Bounce, pChunk->Dyn_StaticFriction, pChunk->Dyn_SlidingFriction);
	break;
...
void main(int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else
		{
			...
			if (!_strcmpi(szOption, "keys"))
				g_bDumpKeys = true;
			else
			if (!_strcmpi(szOption, "mesh"))
			{
			...
			if (!_strcmpi(szOption, "boneInitPos"))
			{
				g_bDumpBoneInitPos  = true;
			}
			else
			if (!_strcmpi (szOption, "meshVerts"))
			{
				g_bDumpMeshVerts = true;
			...
			if (!_strcmpi (szOption, "meshFaces"))
			{
				g_bDumpMeshFaces = true;
			...
			if (!_strcmpi (szOption, "meshTexFaces"))
			{
				g_bDumpMeshTexFaces = true;
			}
			else
			if (!_strcmpi (szOption, "meshBones"))
			{
				g_bDumpMeshBones = true;
			}
			else
			if (!_strcmpi (szOption, "meshVertCols"))
			{
				g_bDumpMeshVertCols = true;
			}
			else
			if (!_strcmpi(szOption, "collectTextures"))
			{
				g_bCollectTextures = true;
			}
			else
			if (!_strcmpi(szOption, "collectTexturesForCopying"))
			{
				g_bCollectTextures = true;
				g_bCollectTexturesForCopying = true;
			}
			else
			if (!_strcmpi(szOption, "briefInfo"))
			{
				g_bBriefInfo = true;
			}
		}
